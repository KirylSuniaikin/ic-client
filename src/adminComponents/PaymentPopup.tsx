...
// Assumed: Actual props are {open, onClose, order, onPaymentSuccess} — spec's branchId/onPaymentComplete don't match the codebase.
...
const toMoney = (n: number): number => Number((Math.round(n * 100) / 100).toFixed(2));
...
export default function PaymentPopup({
                                               ...
                                               order,
                                               onPaymentSuccess
                                           }: PaymentPopupProps): JSX.Element | null {
    ...
    const addPayer = (): void => setPayers(prev => [...prev, { id: mkId(), type: "Cash", amount: "" }]);
    const removePayer = (index: number): void => setPayers(prev => prev.filter((_, i) => i !== index));
    const updatePayerType = (index: number, type: string): void =>
        setPayers(prev => prev.map((p, i) => (i === index ? { ...p, type } : p)));
    const updatePayerAmount = (index: number, value: string): void => {
    ...
    const confirmPayment = async (): Promise<void> => {
        ...
        if (!splitMode) {
            ...
            await sendOrderPayment({
                orderId: order.id,
                amount: amountPaid,
                type: selectedType as import('../types/orderTypes').PaymentType,
                branchId
            });
        } else {
            ...
            for (const tx of txs) {
                await sendOrderPayment({
                    orderId: order.id,
                    amount: tx.amount,
                    type: tx.type as import('../types/orderTypes').PaymentType,
                    branchId
                });
            }
        }
    ...
    const neutralBtn = (selected: boolean) => ({
