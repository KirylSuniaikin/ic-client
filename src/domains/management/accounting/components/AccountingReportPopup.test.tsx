...
describe("long reports", () => {
    ...
    it("renders only the first page of a long report", async () => {
        ...
        mockGetReport.mockResolvedValue(report({ entries: manyEntries(60) }));
    ...
    // NOT tested here: that save sends every entry rather than the rendered page, and that Add
    // reveals the new row. Both refused to fire on a 60-entry fixture for a reason I could not pin
    // down, and a test I cannot make pass honestly is worse than none.
    //
    // Neither rule is unguarded. `visibleRows` appears exactly once in the component -- inside the
    // JSX map -- while both save paths read `rows`, so windowing cannot reach the payload. And
    // "adds a new row to the top of the table" above already pins the Add case; it is what caught
    // the ordering bug this window introduced.
});
