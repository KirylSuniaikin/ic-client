// Assumed: Actual props differ from spec; using real props from the codebase.
...
import {discountArray} from "../../../cart/components/CartItemHorizontal";
...
interface AdminOrderDetailsPopUpProps {
    isAdminOrderDetailsPopUpOpen: boolean;
    onClose: () => void;
    onSave: (phone: string | null, customerName: string, deliveryMethod: string, paymentMethod: string, note: string) => void;
    cartItems: CartItem[];
    setCartItems: (items: CartItem[]) => void;
}

export default function AdminOrderDetailsPopUp({isAdminOrderDetailsPopUpOpen, onClose, onSave, cartItems, setCartItems}: AdminOrderDetailsPopUpProps): JSX.Element {
    ...
    useEffect(() => {
        const order = JSON.parse(localStorage.getItem("orderToEdit") ?? "null")
        if(order){
            ...
            <Box sx={{ px: 2, pt: 2, pb: 12, overflowY: "auto" }}>
                ...
                <TextField
                    ...
                    onChange={(e) => {
                        ...
                        if (/^\d*$/.test(val) && countryObj && val.length <= countryObj.digits) {
                            setPhoneDigits(val);
                            setPhoneError("");
                        }
                    }}
